...
**Kキー**(キー設定「Squad Teleport」カテゴリで変更可)で分隊画面を開ける。内容が画面に収まらない場合はマウスホイールでスクロールできる(右端にスクロールバー表示)。

画面は3つのタブに分かれている:

- **分隊**: 未所属時は分隊作成ボタン・届いている招待の表示(「〇〇から招待が届いています」+承認/拒否)。所属時はメンバー一覧(リーダー★・座標・オンライン状態、1秒ごとに自動更新)+各メンバーへの [TP]・リーダーなら [キック]/[委譲]、続けて集合地点の表示/[移動]/[ここに設定]・リスポーンビーコンの表示/[移動]/残り使用回数、末尾に [脱退]/[解散](テレポート系の操作を1つのタブにまとめてある)
- **募集**: リーダーには「参加申請」一覧([承認]/[拒否])と「招待できるプレイヤー」一覧(タブリスト上のオンラインプレイヤー)+ [招待] ボタン。誰でも「分隊に参加申請する」欄からオンラインプレイヤーへ申請できる
- **設定**: 接近アラートのベル音ON/OFF、および(所属中のリーダーのみ)参加方式の切り替えボタン(自由参加/招待制)
...
「リスポーンビーコン」アイテム(クリエイティブタブ「機能的ブロック」)を右クリックで設置すると、体力1の専用エンティティが現れる。分隊の**3つ目のリスポーン地点**(集合地点・メンバーに加えて)として機能する。

- 誰でも設置可能。1分隊につき同時に1個まで(新しく設置すると古いものは消える)
- 既定4回使用でき、使うたびに残り回数を分隊全員に通知。使い切ると消滅
- **体力1固定**。分隊メンバー・同じscoreboardチームの攻撃は無効化されるが、**敵対Mobまたは別チームプレイヤーの攻撃を1発受けると破壊される**(前線に晒すほどリスクがある拠点)
- `/squad beacon`(平常時、`/squad tp`と同じクールダウン・コスト)/ `/squad respawn beacon`(死亡直後のリスポーン選択画面から、無料・時間窓内のみ)の両方でテレポート可能
- 分隊GUI・リスポーン選択画面(マップにマーカー表示)・JourneyMapウェイポイントに反映される
- `/squad admin disable beacon` でサーバー全体無効化可能。設置数は `beaconUses`(既定4)で調整
...
設定 (`revive` セクション): `downedTimeoutSeconds`(30) / `reviveCastSeconds`(2.5、小数秒指定可・`/squad admin revivetime`で実行時上書き可) / `reviveHealPercent`(30) / `reviveInvulnSeconds`(3) / `allowNonSquadRevive`(false=分隊メンバーのみ蘇生可) / `approachAlertEnabled`(true、サーバーconfig) / `approachAlertRadius`(24、サーバーconfig) / `giveUpHoldTicks`(60=3秒、ダウン中のギブアップキー長押し時間、20tick=1秒。蘇生の詠唱時間(既定5秒)より短すぎない値にして、諦める方が確実に速い状態を避けている)
...
- `beacon.beaconUses` (既定 4) — リスポーンビーコン1個あたりの使用可能回数
